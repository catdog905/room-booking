...
from ...domain.entities.time.time_stamp import TimeStamp, CurrentTimeStamp, ShiftedBackwardTimeStamp, \
    ShiftedForwardTimeStamp
...
async def get_my_bookings(
    ...
    bookings_with_id = await adapter.get_bookings_in_period(
        period=TimePeriod(
            start=ShiftedBackwardTimeStamp(current_time, timedelta(days=365)),
            end=ShiftedForwardTimeStamp(current_time, timedelta(days=365))
        ),
        filter_rooms=None,
    ...
    return [BookingWithIdSchema.from_booking_with_id(booking_with_id, language) for booking_with_id in bookings_with_id
            if booking_with_id.owner.email == user.email]
